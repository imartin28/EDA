...
bool resuelveCaso() {
    ...
    while(operacion != "FIN"){
        if(operacion == "nuevo"){
            ...
            std::cin >> puntos;
            try {
                std::list<std::string> lista_dni = carnet.lista_por_puntos(puntos);
                std::cout << "Tienen " << puntos << " puntos: ";
                for(auto it = lista_dni.begin(); it != lista_dni.end(); ++it){
                    std::cout << *it << ' ';
                }
                std::cout << std::endl;
...
int main() {
    ...
    std::ifstream in("/Users/IRENE/Desktop/EDA_2 EXAMEN/txt/eda35.txt");
