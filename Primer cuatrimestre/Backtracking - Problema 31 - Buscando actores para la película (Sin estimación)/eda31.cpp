...
void resolver(std::vector<std::vector<int>>const& matriz, std::vector<int>const& v_sueldo_actor,  std::vector<bool>& marcas_actor_ocupado, int const numPapeles, int k_indicePapeles, int const presupuestoTotal, int cont_puntuacion_total, int cont_presupuesto, int const puntuacionMin, int& maxPuntuacionTotal, int& presupuestoSolucion,  std::vector<tSol>& v_solucion,  std::vector<tSol>& v_papelesAsignados ) {
    ...
    for(int indiceActor = 0; indiceActor < matriz[0].size(); ++indiceActor){
        ...
        if(esValida(marcas_actor_ocupado, indiceActor, cont_presupuesto, presupuestoTotal, cont_puntuacion_total, puntuacionMin)){
            ...
            if(k_indicePapeles == numPapeles - 1){      //esSolucion
                ...
                resolver(matriz, v_sueldo_actor, marcas_actor_ocupado, numPapeles, k_indicePapeles + 1, presupuestoTotal, cont_puntuacion_total, cont_presupuesto, puntuacionMin, maxPuntuacionTotal, presupuestoSolucion, v_solucion, v_papelesAsignados);
...
bool resuelveCaso() {
    ...
    resolver(matriz, v_sueldo_actor,  marcas_actor_ocupado, numPapeles,  indicePapeles, presupuestoTotal, cont_puntuacion_total, cont_presupuesto, puntuacionMin, maxPuntuacionTotal, presupuestoSolucion, v_solucion, v_papelesAsignados);
